...
describe('NinjaClient.getBuildsForSkill', () => {
  it('throws with a clear message about the moved API', async () => {
    const ctx = makeCtx();
    const client = new NinjaClient(ctx);

    await expect(client.getBuildsForSkill('Boneshatter', null, 'Standard')).rejects.toThrow(
      'poe.ninja builds API has changed'
    );
  });
  ...
  it('does not make any HTTP calls', async () => {
    ...
    await client.getBuildsForSkill('Boneshatter', null, 'Standard').catch(() => {});
